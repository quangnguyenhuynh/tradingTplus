...
def _complete(**changes):
    return {
        ...
        "stock_daily_count": 2, "missing_stock_daily_count": 0, "status": "OK",
    } | changes
...
def test_stock_eod_runs_only_stock_stages_in_order(monkeypatch):
    ...
    assert not hasattr(stock_eod, "run_intraday_ingest")
...
def test_stock_eod_status_depends_only_on_stock_stages(monkeypatch):
    _install(monkeypatch, completeness=_complete(status="PARTIAL", missing_stock_daily_count=1))
