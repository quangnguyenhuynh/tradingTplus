...
from src.analogs.profile import AnalogProfile, config_hash, load_profile, load_source_profile, validate_profile
from src.analogs.runtime import exact_profile
from src.analogs.validation import walk_forward


V1_HASH = "22ec92b1ce460106fa5d388bccf5e522fc33812fd38eecc8117eb24a3b1c5188"


def test_v1_unchanged_and_v2_exact_deterministic_profiles():
    ...
    assert v1.config_hash == V1_HASH
    assert v2.config_hash == config_hash(deepcopy(v2.config)) != v1.config_hash
    assert v1_bytes == Path("src/analogs/profiles/tplus_analog_core_eod_v1.json").read_bytes()
...
def test_h10_is_tenth_observed_session_pending_and_invalid_is_unavailable():
    ...
    assert resolve_outcomes(sessions[0], 100, sessions[:10], closes, [10])[0]["status"] == "pending"
    closes[sessions[10]] = float("nan")
    unavailable = resolve_outcomes(sessions[0], 100, sessions, closes, [10])[0]
    assert unavailable["status"] == "unavailable" and unavailable["reason"] == "INVALID_CLOSE"
...
def feature_rows(sessions):
    ...
    for index, session in enumerate(sessions):
        rows.append({
            "symbol": "SSI", "timeframe": "1d", "trading_session": session,
            "close": 100 + index, "low": 99 + index, "high": 102 + index,
            "ema20": 90 + index, "ema50": 80 + index, "rsi14": 50 + index,
            "macd_histogram": 1, "high_20_bars": 110 + index,
            "volume_ratio": 1 + index / 10, "value_ratio": 1 + index / 20,
        })
    return rows
...
def test_evaluable_v2_snapshot_builds_four_outcome_records():
    ...
    result = build_history(profile, features, sessions, closes, symbols=["SSI"], start=sessions[5], end=sessions[5], mode="full")
    assert [row["horizon_sessions"] for row in result["outcomes"]] == [1, 3, 5, 10]


def approved_v2():
    source = load_source_profile("TPLUS_ANALOG_CORE_EOD", 2)
    config = {**source.config, "distance_threshold": 999, "status": "approved", "minimum_sample": 2, "top_k": 2}
    return AnalogProfile(config, config_hash(config))


def dimensions(seed):
    return {name: seed * (index + 1) for index, name in enumerate(load_profile().weights)}


def snapshot(profile, index, session, *, h10_target=None):
    return {
        "id": str(index), "profile_code": profile.code, "version": profile.version,
        "config_hash": profile.config_hash, "symbol": "SSI", "timeframe": "1d",
        "checkpoint": "EOD", "trading_session": session, "status": "evaluable",
        "dimensions": dimensions(index + 1),
        "outcomes": {h: {"status": "completed", "target_session": h10_target if h == 10 and h10_target else session + timedelta(days=h), "return_ratio": 0.01 * h} for h in profile.config["horizons"]},
    }
...
def test_v2_h10_observability_controls_candidates_statistics_and_validation_reasons():
    ...
    candidates = [snapshot(profile, i, date(2025, 1, 1) + timedelta(days=i * 20)) for i in range(4)]
    ...
    result = match_snapshot(current, candidates + [leaked], profile, production=False, query_cutoff=cutoff)
    assert result["status"] == "completed"
    assert set(result["statistics"]) == {"1", "3", "5", "10"}
    assert "future-h10" not in {row["snapshot_id"] for row in result["matches"]}

    current["outcomes"][10] = {"status": "pending", "reason": "TARGET_SESSION_NOT_YET_OBSERVABLE"}
    ...
    assert evidence["reason_counts"]["CURRENT_OUTCOME_H10:TARGET_SESSION_NOT_YET_OBSERVABLE"] == 1
...
def test_v1_completed_statistics_remain_three_horizons():
    ...
    cutoff = date(2026, 1, 31)
    candidates = [snapshot(profile, i, date(2025, 1, 1) + timedelta(days=i * 20)) for i in range(4)]
    result = match_snapshot(snapshot(profile, 20, cutoff), candidates, profile, query_cutoff=cutoff)
