...
def _resolve_as_of(target: date, as_of=None) -> pd.Timestamp:
    ...
    if as_of is None:
        if target < now.date():
            return pd.Timestamp(datetime.combine(target, datetime.max.time(), tzinfo=VN_TZ))
        if target > now.date():
    ...
    if ":" in text and "T" not in text and " " not in text:
        try:
            ...
            raise ValueError("as_of HH:MM must be valid Vietnam local time") from exc
        stamp = pd.Timestamp(datetime.combine(target, parsed_time, tzinfo=VN_TZ))
    else:
...
def _bucket_close_local(start: pd.Timestamp, timeframe: str) -> pd.Timestamp:
    minutes = int(timeframe[:-1])
    normal = start + pd.Timedelta(minutes=minutes)
    local_date = start.date()
    morning_end = pd.Timestamp(datetime.combine(local_date, datetime.strptime("11:30", "%H:%M").time(), tzinfo=VN_TZ))
    afternoon_end = pd.Timestamp(datetime.combine(local_date, datetime.strptime("15:00", "%H:%M").time(), tzinfo=VN_TZ))
    boundary = morning_end if start < morning_end else afternoon_end
    return min(normal, boundary)


def filter_closed_buckets(df: pd.DataFrame, timeframe: str, as_of: pd.Timestamp) -> pd.DataFrame:
    """Keep observed buckets whose configured Vietnam-session close has passed."""
    if df.empty:
        return df.copy()
    starts = pd.to_datetime(df['time'], utc=True).dt.tz_convert(VN_TZ)
    closed = starts.map(lambda start: _bucket_close_local(start, timeframe) <= as_of)
    return df.loc[closed.to_numpy()].reset_index(drop=True)


def calculate_daily_features_for_symbol(symbol, mode='full', target_date=None, upsert_batch_size=1000, records_by_timeframe=None):
    db = SupabaseClient()
    target = _normalize_target_date(target_date) if mode == 'incremental' else None
    # All available prior daily history is intentional in Phase 0 for full/incremental parity.
    rows = _fetch_stock_daily_rows(db, symbol, end_date=target)
    if not rows:
        return 0
    frame = compute_daily_features(pd.DataFrame(rows))
    if target is not None:
        start, end, _ = _target_utc_bounds(target)
        frame = _filter_output_by_time(frame, start, end)
    count = _upsert_feature_frame(db, symbol, '1d', frame, upsert_batch_size)
    if records_by_timeframe is not None:
        records_by_timeframe['1d'] = records_by_timeframe.get('1d', 0) + count
    _log_feature_run(symbol, '1d', mode, len(rows), len(frame), count, frame)
    return count


def calculate_intraday_features_for_symbol(symbol, timeframes=None, mode='full', target_date=None, as_of=None, upsert_batch_size=1000, records_by_timeframe=None):
    normalized = _normalize_timeframes(timeframes or tuple(INTRADAY_TIMEFRAMES))
    if '1d' in normalized:
        raise ValueError("Intraday feature flow does not accept 1d")
    db = SupabaseClient()
    target = _normalize_target_date(target_date) if mode == 'incremental' else None
    # Phase 0 correctness: fetch all available prior 1m history, paginated and bounded
    # by source exhaustion. This guarantees EWM parity without persisted indicator state.
    end = None
    filter_start = filter_end = None
    if target is not None:
        filter_start, filter_end, _ = _target_utc_bounds(target)
        end = filter_end.strftime('%Y-%m-%dT%H:%M:%SZ')
    rows = _fetch_stock_intraday_paginated(db, symbol, lt_time=end, order_desc=False)
    if not rows:
        return 0
    bounds = _date_bounds_for_daily_context(rows)
    daily_rows = _fetch_stock_daily_rows(db, symbol, bounds[0], bounds[1]) if bounds else []
    cutoff = _resolve_as_of(target or pd.to_datetime(rows[-1]['time'], utc=True).tz_convert(VN_TZ).date(), as_of)
    total = 0
    source = pd.DataFrame(rows)
    observed_dates = source['time'].pipe(pd.to_datetime, utc=True).dt.tz_convert(VN_TZ).dt.date.nunique()
    for timeframe in normalized:
        ...
        output = _filter_output_by_time(computed, filter_start, filter_end)
        count = _upsert_feature_frame(db, symbol, timeframe, output, upsert_batch_size)
        total += count
        if records_by_timeframe is not None:
            records_by_timeframe[timeframe] = records_by_timeframe.get(timeframe, 0) + count
        logger.info("Intraday warm-up symbol=%s timeframe=%s source_range=%s..%s aggregated=%s prior_dates=%s sufficient=%s",
                    symbol, timeframe, source['time'].min(), source['time'].max(), len(aggregated), observed_dates,
                    len(aggregated) >= 50 and observed_dates >= 21)
        _log_feature_run(symbol, timeframe, mode, len(rows), len(computed), count, output)
    return total


def _run_source_summary(flow, symbols, mode, target_date, timeframes, per_symbol, as_of=None):
    db = SupabaseClient()
    if not db.health_check():
        raise RuntimeError("Supabase health-check failed. Please check connection and credentials.")
    if symbols is None:
    ...
    if mode not in {'full', 'incremental'}:
        raise ValueError("mode must be either 'full' or 'incremental'")
    target = _normalize_target_date(target_date) if mode == 'incremental' else None
    records = {tf: 0 for tf in timeframes}
    errors, total, successes = [], 0, 0
    for symbol in symbols:
        try:
            kwargs = dict(symbol=symbol, mode=mode, target_date=target, records_by_timeframe=records)
            ...
            errors.append({'symbol': symbol, 'error': str(exc)})
    failed = len(errors)
    status = 'FAILED' if failed == len(symbols) or total == 0 else ('PARTIAL' if failed else 'OK')
    return {'flow': flow, 'mode': mode, 'target_date': target.isoformat() if target else None,
            'requested_symbols': len(symbols), 'successful_symbols': successes, 'failed_symbols': failed,
            'total_records': total, 'records_by_timeframe': records, 'errors': errors, 'status': status}


def run_daily_features_with_summary(symbols=None, mode='incremental', target_date=None):
    return _run_source_summary('features-daily', symbols, mode, target_date, ('1d',), calculate_daily_features_for_symbol)


def run_intraday_features_with_summary(symbols=None, mode='incremental', timeframes=None, target_date=None, as_of=None):
    normalized = _normalize_timeframes(timeframes or ('1m', '5m', '15m', '60m'))
    if '1d' in normalized:
        raise ValueError("features-intraday accepts only 1m, 5m, 15m, and 60m")
    return _run_source_summary('features-intraday', symbols, mode, target_date, normalized, calculate_intraday_features_for_symbol, as_of)


def run_feature_engine_with_summary(symbols=None, mode='full', timeframes=None, target_date=None, as_of=None):
    normalized = _normalize_timeframes(timeframes)
    daily_requested = '1d' in normalized
    intraday_requested = tuple(tf for tf in normalized if tf != '1d')
    summaries = []
    if daily_requested:
        summaries.append(run_daily_features_with_summary(symbols, mode, target_date))
    if intraday_requested:
        summaries.append(run_intraday_features_with_summary(symbols, mode, intraday_requested, target_date, as_of))
    if len(summaries) == 1:
        result = dict(summaries[0])
        result['compatibility_warning'] = 'features is a compatibility orchestrator; prefer features-daily or features-intraday.'
    ...
    return {'flow': 'features', 'mode': mode, 'target_date': summaries[0]['target_date'] if summaries else None,
            'total_records': total, 'records_by_timeframe': {k: v for summary in summaries for k, v in summary['records_by_timeframe'].items()},
            'source_summaries': summaries, 'errors': errors,
            'status': 'FAILED' if statuses == {'FAILED'} else ('PARTIAL' if 'FAILED' in statuses or 'PARTIAL' in statuses else 'OK'),
            'compatibility_warning': 'features is a compatibility orchestrator; prefer features-daily or features-intraday.'}


def run_feature_engine(symbols=None, mode='full', timeframes=None, target_date=None):
    return run_feature_engine_with_summary(symbols, mode, timeframes, target_date)['total_records']
...
def calculate_features_for_symbol_full_chunked(symbol, timeframes=None, upsert_batch_size=1000):
    ...
    total = 0
    if '1d' in normalized:
        total += calculate_daily_features_for_symbol(symbol, 'full', upsert_batch_size=upsert_batch_size)
    intraday = tuple(tf for tf in normalized if tf != '1d')
    if intraday:
        total += calculate_intraday_features_for_symbol(symbol, intraday, 'full', upsert_batch_size=upsert_batch_size)
    return total


def calculate_features_for_symbol_incremental(symbol, timeframes=None, target_date=None, warmup_bars=None, upsert_batch_size=1000, records_by_timeframe=None):
    normalized = _normalize_timeframes(timeframes)
    total = 0
    if '1d' in normalized:
        total += calculate_daily_features_for_symbol(symbol, 'incremental', target_date, upsert_batch_size, records_by_timeframe)
    intraday = tuple(tf for tf in normalized if tf != '1d')
    if intraday:
        total += calculate_intraday_features_for_symbol(symbol, intraday, 'incremental', target_date, None, upsert_batch_size, records_by_timeframe)
    return total


def calculate_features_for_symbol(symbol, timeframe='1m'):
    return calculate_features_for_symbol_full_chunked(symbol, [timeframe])
