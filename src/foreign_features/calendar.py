"""Verified-session calendar adapter. Weekdays are deliberately not inferred."""
...
def load_calendar(path: str | None) -> TradingCalendar | None:
    ...
    if not isinstance(sessions, list) or not sessions or not source or not market:
    ...
    identity = hashlib.sha256(json.dumps({"sessions": normalized, "source": source, "market": market}, separators=(",", ":")).encode()).hexdigest()
