...
def _paged_stock_daily(db: Any, columns: str, symbols: list[str], *, start: str | None = None, end: str | None = None) -> list[dict]:
    rows: list[dict] = []
    for offset in range(0, 10_000_000, PAGE_SIZE):
        query = db.client.table("stock_daily").select(columns).in_("symbol", symbols)
...
def load_session_dates(db: Any, symbols: list[str], *, start: str | None = None, end: str | None = None) -> tuple[str, ...]:
