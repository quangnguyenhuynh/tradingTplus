...
class AnalogRepository:
    ...
    def list_profiles(self) -> list[dict[str, Any]]:
        return self._pages(lambda: self.client.table("analog_profiles").select("*").order("profile_code").order("version"))

    def get_profile(self, code: str, version: int) -> dict[str, Any] | None:
        rows = self.client.table("analog_profiles").select("*").eq("profile_code", code).eq("version", version).limit(1).execute().data or []
    ...
    def register_profile(self, row: dict[str, Any]) -> None:
        ...
        self.client.table("analog_profiles").upsert(self._safe(row), on_conflict="profile_code,version").execute()
    ...
    def upsert_snapshots(self, rows: Iterable[dict[str, Any]]) -> int:
        ...
        for start in range(0, len(values), self.page_size):
            self.client.table("analog_snapshots").upsert(values[start:start+self.page_size], on_conflict="profile_code,version,config_hash,symbol,timeframe,checkpoint,trading_session").execute()
        return len(values)

    def resolve_snapshot_ids(self, *, code: str, version: int, config_hash: str, symbols: list[str], start: str, end: str) -> list[dict[str, Any]]:
        return self._pages(lambda: self.client.table("analog_snapshots").select("id,config_hash,symbol,trading_session").eq("profile_code", code).eq("version", version).eq("config_hash", config_hash).in_("symbol", symbols).eq("timeframe", "1d").eq("checkpoint", "EOD").gte("trading_session", start).lte("trading_session", end).order("trading_session"))
    ...
    def upsert_outcomes(self, rows: Iterable[dict[str, Any]]) -> int:
        ...
        for source in rows:
            ...
            values.append(self._safe(row))
        for start in range(0, len(values), self.page_size):
            self.client.table("analog_outcomes").upsert(values[start:start+self.page_size], on_conflict="snapshot_id,horizon_sessions").execute()
        return len(values)
    ...
    def replace_scope(self, **scope: Any) -> int:
        ...
        result = self.client.table("analog_snapshots").delete().eq("profile_code", scope["code"]).eq("version", scope["version"]).eq("config_hash", scope["config_hash"]).in_("symbol", symbols).eq("timeframe", "1d").eq("checkpoint", "EOD").gte("trading_session", start).lte("trading_session", end).execute()
        return len(result.data or [])

    def fetch_snapshots(self, *, code: str, version: int, config_hash: str, symbol: str, through: date) -> list[dict[str, Any]]:
        return self._pages(lambda: self.client.table("analog_snapshots").select("*").eq("profile_code", code).eq("version", version).eq("config_hash", config_hash).eq("symbol", symbol).eq("timeframe", "1d").eq("checkpoint", "EOD").lte("trading_session", through.isoformat()).order("trading_session"))
    ...
    def fetch_outcomes(self, snapshot_ids: list[str]) -> list[dict[str, Any]]:
        ...
        for start in range(0, len(snapshot_ids), 200):
            ...
            if ids:
                rows.extend(self._pages(lambda ids=ids: self.client.table("analog_outcomes").select("*").in_("snapshot_id", ids).order("snapshot_id").order("horizon_sessions")))
        return rows
    ...
    def latest(self, symbol: str, checkpoint: str = "EOD") -> dict[str, Any] | None:
        rows = self.client.table("analog_queries").select("*").eq("symbol", symbol).eq("checkpoint", checkpoint).order("as_of_session", desc=True).limit(1).execute().data or []
        return rows[0] if rows else None

    def query_detail(self, query_id: str) -> dict[str, Any] | None:
        rows = self.client.table("analog_queries").select("*").eq("id", query_id).limit(1).execute().data or []
        if not rows:
            return None
        matches = self._pages(lambda: self.client.table("analog_query_matches").select("*").eq("query_id", query_id).order("rank"))
