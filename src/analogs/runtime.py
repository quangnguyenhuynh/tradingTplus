    ...
    *,
...
) -> tuple[list[dict[str, Any]], dict[str, list[date]], dict[tuple[str, date], Any]]:
    ...
    for symbol in symbols:
        feature_rows = _paged(
            ...
            .eq("symbol", symbol)
            .eq("timeframe", "1d")
            .gte("time", f"{warm_start.isoformat()}T00:00:00+07:00")
            .lte("time", f"{end.isoformat()}T23:59:59.999999+07:00")
            .order("time")
        )
...
def exact_profile(
    repository: AnalogRepository,
    code: str,
    version: int,
    requested_hash: str | None = None,
) -> tuple[AnalogProfile, dict[str, Any]]:
    ...
    if (
        row.get("config_hash") != source.config_hash
        or row.get("configuration") != source.config
    ):
...
def hydrate_evidence(
    repository: AnalogRepository, profile: AnalogProfile, symbol: str, session: date
) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    rows = repository.fetch_snapshots(
        code=profile.code,
    ...
    for outcome in outcomes:
        ...
        grouped.setdefault(item["snapshot_id"], {})[
    ...
    for source in rows:
        row = dict(source)
        row["trading_session"] = (
            date.fromisoformat(row["trading_session"])
            if not isinstance(row["trading_session"], date)
            else row["trading_session"]
        )
...
def query_persisted(
    ...
    *,
    ...
    reasons = []
    if profile.config["status"] != "approved":
    ...
    if not current:
        return {
            ...
            "reason_codes": ["CURRENT_SNAPSHOT_NOT_FOUND"],
            "persisted": False,
        }
    ...
    if apply and result["status"] in {
        ...
        qfp = fingerprint(
            {
                "snapshot": current["input_fingerprint"],
        ...
        query = {
            ...
            "usable_sample": result.get("usable_sample", 0),
            "normalization_parameters": result.get("normalization"),
            "result_statistics": {
                "outcomes": result.get("statistics"),
                "analog_quality": result.get("analog_quality"),
                "statistical_confidence": result.get("statistical_confidence"),
                "warnings": result.get("warnings", []),
            },
            "baseline_statistics": {
                h: s.get("baseline_probability")
                for h, s in result.get("statistics", {}).items()
            },
            "input_fingerprint": current["input_fingerprint"],
            "query_fingerprint": qfp,
            "engine_version": f"historical-analog-eod-v{profile.version}",
            "executed_at": app_now_iso(),
        }
        matches = [
            {
                "rank": m["rank"],
                "matched_snapshot_id": m["snapshot_id"],
                "distance": m["distance"],
                "similarity": m["similarity"],
                "normalized_differences": m["normalized_differences"],
            }
...
def inspect(
    ...
    *,
    ...
    start = session.replace(
        year=session.year - int(profile.config["maximum_lookback_years"])
    )
    features, sessions, closes = read_inputs(
        repository.client,
        [symbol],
    ...
    for outcome in built["outcomes"]:
        outcome_by_key.setdefault(outcome["snapshot_key"], {})[
    ...
    for index, snapshot in enumerate(snapshots):
        ...
        row["outcomes"] = outcome_by_key.get(
            (profile.config_hash, symbol, row["trading_session"]), {}
        )
        hydrated.append(row)
    current = next((row for row in hydrated if row["trading_session"] == session), None)
    if not current:
        return {
            "status": "not_evaluable",
            "reason_codes": ["CURRENT_FEATURE_NOT_FOUND"],
            "persisted": False,
            "production": False,
        }
    ...
    result = match_snapshot(
        current,
        [r for r in hydrated if r["trading_session"] < session],
        research,
        production=False,
        query_cutoff=session,
    )
    result.update(
        current_snapshot={
            "symbol": symbol,
            ...
            "invalid_reason_codes": current["invalid_reasons"],
        },
        persisted=False,
        production=False,
        ignored_legacy_distance_threshold=threshold,
        disclaimer="Non-production research/debug output; not persisted and not approval evidence or investment advice",
    )
    return result
