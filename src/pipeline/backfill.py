...
def backfill(
    ...
    if symbols:
        raise ValueError("symbol-scoped backfill is unsupported because EOD has no symbol scope")
    if allow_future:
    ...
    return run_backfill_pipeline(normalize(from_date), normalize(to_date))
