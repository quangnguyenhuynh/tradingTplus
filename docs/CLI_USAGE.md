...
## Dormant fixed-rule commands

The following command groups still exist in code:
...
They implement the superseded `rule -> backtest -> approve -> signal` research
flow. Do not use their write or approval paths for production, and do not treat
their metrics as historical-analog evidence. They remain documented only so the
current executable repository is not misrepresented.

## Accepted Phase 1 direction

The target historical-analog flow is specified in
[`phase1/HISTORICAL_ANALOG_SPEC.md`](phase1/HISTORICAL_ANALOG_SPEC.md): one
symbol uses only its own prior states at the same checkpoint. Its proposed
`analogs` CLI and database tables are **not implemented**. No command shown in
that specification should be run until an implementation task adds code,
migrations, tests, and an explicit historical-build scope.
